...
- 喀痰検査 `6A010` / `6A205` は、一般受診者の一律必須ではなく、胸部X線等で所見がある場合などの条件付き項目として扱う。
- Phase7では条件付き必須をまだ表現しないため、喀痰検査 `6A010` / `6A205` は `required_flag = 0` とする。
- 喀痰検査 `6A010` / `6A205` は条件付き必須候補として扱い、条件付き判定実装までは法定NG理由にしない。
...
- Phase7では、`9N016` が有効値として存在する場合、`9N021` を `ALTERNATIVE` として扱う。
