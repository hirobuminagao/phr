...
## DH-20260709-10 / 2026-07-09 16:58 JST

### テーマ
Phase7制度チェック実行単位と設定ファイル責務の統一

### 背景
Phase7を実データで実行する中で、`event_id` をCLI引数で毎回指定する運用と、YAML設定ファイルから取得する運用を再検討した。

本プロジェクトではPhase4以降、各スクリプトが設定ファイルから環境・実行条件を取得する構成を採用しており、Phase7のみ実行対象をCLIで指定する設計となっていたため、運用方針を統一する必要があった。

### 議論
- 実行環境では同一イベントに対して繰り返し制度チェックを実施するケースが多い。
- 毎回 `--event-id` を指定するより、設定ファイルから取得する方が運用負荷が小さい。
- Phase4～Phase7で設定取得方法を統一した方が保守性・運用性が高い。
- CLIは実行開始やデバッグ用オプションを受け付ける責務とし、通常の実行対象は設定ファイルから取得する。
- 将来的に `xml_ledger_id` や `subscriber_id` 単位の再実行を追加する場合は、CLIオプションとして追加する。

### 現時点の考え
通常運用では設定ファイルをSingle Source of Truthとし、実行対象も設定ファイルから取得する。

CLIは一時的なデバッグや限定実行など、通常運用とは異なるケースのためのインターフェースとする。

### 決定事項
- `event_id` はYAML設定ファイルで管理する。
- Phase7は設定ファイルから `event_id` を取得して実行する。
- Phase4～Phase7で設定取得方法を統一する。
- CLIはデバッグ・限定実行など補助用途として利用する。

### 保留事項
- `--xml-ledger-id`、`--subscriber-id` など個別再実行CLIの設計。
- 将来的なバッチ実行インターフェース。

### 根拠
- Phase4以降の設定取得方法と統一できるため。
- 通常運用では同一イベントを繰り返し実行するケースが多く、毎回CLI指定する必要性が低いため。
- 設定方法を統一した方が保守性・運用性が向上するため。

### 実装引き継ぎ
- `03_decisions.md` へ同期後、実装者は `event_id` をYAML設定ファイルから取得する実装とする。
- Phase4～Phase7は共通して設定ファイルを読み込む構成とする。
- CLIは通常運用の入力ではなく、デバッグ・限定実行用インターフェースとして扱う。

### 次回検討
- `03_decisions.md` へ本DHの決定事項を同期する。
- `03_check_exam_results.py` を設定ファイルから `event_id` を取得する構成へ変更する。
- 将来的な個別再実行CLI（`--xml-ledger-id`、`--subscriber-id` 等）の設計を整理する。

---

## DH-20260709-11 / 2026-07-09 18:28 JST

### テーマ
Phase7制度チェックにおける暫定実装とルールマスタ整備方針

### 背景

Phase7の dry-run を実施した結果、制度チェック自体は動作しているものの、現行ルールマスタ（seed）では制度仕様を十分に表現できていない項目が確認された。

特に、代替項目・条件付き必須・任意項目・未実装ルールなどが単純な `required_flag` だけでは表現できず、実際の制度判定と乖離するケースが見つかった。

### 議論

- Phase7の目的は制度チェック基盤を完成させることであり、まずは実装を優先する。
- 現時点ではルールマスタを大きく変更せず、スクリプト側で制度判定を補完する。
- 代替判定、条件付き必須、未実装ルールなどはスクリプトで吸収する。
- 一方で、本来これらの制度ルールはスクリプトへ埋め込むものではなく、ルールマスタ（seed）で管理すべき責務である。
- 今回の実装・dry-runで判明した例外条件は、今後ルールマスタへ取り込めるよう整理していく。
- スクリプトで対応した内容は、将来seedへ移管できるよう、実装内容と制度ルールの対応関係を明確に保つ。

### 現時点の考え

Phase7では制度チェックロジックを安定させることを優先する。

制度ルールの完全な表現は後続フェーズでルールマスタへ寄せるが、現時点ではスクリプト側で制度仕様との差異を吸収する。

### 決定事項

- Phase7ではスクリプト側で制度判定を補完する。
- ルールマスタ（seed）の大規模見直しは現時点では実施しない。
- スクリプトへ追加した制度判定は、将来ルールマスタへ移管することを前提とする。
- 制度仕様との差異が判明した場合は、スクリプトで暫定対応し、その内容を設計へ記録する。
...
- 条件付き必須の正式な保持方法。
- 任意項目の集約ルール。
- ルールマスタだけで制度判定を完結できるテーブル設計。

### 根拠

- Phase7 dry-run結果。
- 制度チェック実装レビュー。
- 現行ルールマスタでは制度仕様を十分表現できないことが確認されたため。

### 次回検討

- dry-runで判明した例外条件を整理する。
- ルールマスタへ移管すべき制度ルール一覧を作成する。
- 将来的なルールマスタ再設計を検討する。

---

## DH-20260709-12 / 2026-07-09 19:03 JST

### テーマ
Phase7制度チェックにおける代替ルールと暫定実装の責務整理

### 背景

Phase7の実DB dry-run結果を確認したところ、`9N021（内臓脂肪面積）` が全件 `MISSING` となり、法定・特定健診ともNG判定となっていた。

調査の結果、XMLには `9N016（腹囲）` が正常に取り込まれており、スクリプト側の取得不具合ではなく、現行ルールマスタ（seed）が `9N021` と `9N016` の代替関係を表現できていないことが原因であることを確認した。

### 議論

- `9N021` が全件 `MISSING` となる原因は、XML取込や値取得処理ではなかった。
- 共通Ruleライブラリは `ALTERNATIVE` ルールに既に対応している。
- 現行seedには `9N021` の代替ルールが登録されておらず、単純必須項目として判定されている。
- Phase7ではルールマスタの大規模見直しは実施せず、制度チェック実装を優先する。
- そのため、当面はスクリプト側で制度仕様との差異を吸収する。
- 一方、本来このような制度ルールはスクリプトではなくルールマスタ（seed）が責務を持つべきである。
- 今回判明した代替・条件付き必須などの制度ルールは、将来的にルールマスタへ移管できるよう整理していく。

### 現時点の考え

Phase7では制度チェック基盤の完成を優先する。

制度仕様との差異はスクリプト側で暫定対応するが、制度ルールそのものはルールマスタで管理することを最終形とする。

### 決定事項

- `9N021` の現象はXML取込不具合ではなく、ルールマスタ不足によるものと判断する。
- Phase7ではスクリプト側で制度ルールを暫定補完する。
- `ALTERNATIVE` 等の制度ルールは将来的にルールマスタ（seed）へ移管する。
- スクリプトへ追加した暫定ルールは、後からseedへ移行できるよう責務を明確にして実装する。

### 保留事項

- `9N021` と `9N016` の正式な代替ルール表現方法。
- 代替・条件付き必須をルールマスタで管理する正式設計。
- ルールマスタとスクリプト実装の対応表作成方法。

### 根拠

- Phase7 dry-run結果。
- 実DB確認により `9N016` の取込を確認したこと。
- Ruleライブラリは `ALTERNATIVE` に対応済みであり、seed側にルール定義が存在しないことを確認したため。
...
## DH-20260710-03 / 2026-07-10 20:35 JST

### テーマ
法定健診項目の `required_flag` 付与方針整理

### 背景
Phase7の制度チェック確認中、`9E160（視力）` が労働安全衛生法側の必須項目であるにもかかわらず、`v2_2026_LSIO_Legal_Item` へ登録されていないことが判明した。

また、付属2の記号をそのまま扱うのではなく、プロジェクト側で整理した `02_exam_check_item_spec_v2_0_0.md` を正として、法定健診側の `required_flag` 付与方針を明確にする必要が生じた。

### 議論
- `9E160（視力）` は労働安全衛生法側の必須項目であり、ルールマスタへの登録漏れだった。
- `9E160` は `exam_item_master` 上で `xml_method_code` を持たないため、`exam_item_group_members` の namecode 定義で判定するのが適切であることを確認した。
- 付属2上の `□` は、プロジェクト側で整理した仕様書では `△` として扱っている。
- 今回の `required_flag` 付与方針は、厚労省付属2の記号そのものではなく、`02_exam_check_item_spec_v2_0_0.md` で整理した記号を正とする。
- 労働安全衛生法（法定健診）側で `△` として整理した項目は、一旦 `required_flag = 1` として扱う。
- この方針は法定健診側のみを対象とし、特定健診側の `△` には適用しない。
- `☆（情報を入手した場合に限り報告）` は本方針の対象外とし、`required_flag = 0` を維持する。
- 年齢・性別・医師判断等の条件付き判定は、将来ルールマスタで表現する。

### 現時点の考え
法定健診判定の基準を明確にするため、`02_exam_check_item_spec_v2_0_0.md` の法定健診側整理を正とする。

労働安全衛生法側で `△` として整理した項目は、Phase7では一旦 `required_flag = 1` として扱う。ただし、この決定は特定健診側の `△` には適用しない。

### 決定事項
- Phase7では、本仕様書（`02_exam_check_item_spec_v2_0_0.md`）の法定健診整理を正とする。
- 労働安全衛生法（法定健診）側で「△」として整理した項目は、一旦 `required_flag = 1` として扱う。
- 本仕様書の「△」は、元資料の「□（医師判断等）」を制度チェック用に整理した表現を含む。
...
### 議論（追記）
- 法定健診側△項目の見直しにより、喀痰検査（`6A010` / `6A205`）が `required_flag=1` となっていた。
- ただし喀痰検査は一般受診者に対する一律必須項目ではなく、胸部X線等で所見がある場合などに実施される条件付き項目である。
- Phase7では条件付き必須をまだ表現しないため、喀痰検査は一旦 `required_flag=0` とする。
- `9N021（内臓脂肪面積）` は過去協議で、`9N016（腹囲）` が有効値として存在する場合は `ALTERNATIVE` として扱い、単独MISSINGで法定NGにしない方針としていた。
- そのため、`9N021` が `required_flag=1` になっていても、Phase7判定では `9N016` による暫定代替を維持する。
- `9N021` の正式な代替ルール表現は、将来ルールマスタへ移管する。

### 決定事項（追記）
- 喀痰検査（`6A010` / `6A205`）は、Phase7では `required_flag=0` とする。
- 喀痰検査は条件付き必須候補として扱い、条件付き判定実装までは法定NG理由にしない。
- `9N021（内臓脂肪面積）` は、`9N016（腹囲）` が有効値として存在する場合、Phase7では `ALTERNATIVE` として扱う。
- `9N021` の `9N016` 代替は既存の暫定スクリプト対応を維持し、将来ルールマスタへ移管する。

### 保留事項
- 労働安全衛生法側 `△` のうち、将来的に条件付き必須として詳細判定へ移行すべき項目の整理。
- 年齢・性別・医師判断等の条件付き判定をルールマスタでどう表現するか。
- 特定健診側 `△` の扱い整理。

### 根拠
- `9E160（視力）` は労働安全衛生法側の必須項目であり、法定健診判定に含める必要があるため。
- プロジェクトでは付属2の記号をそのまま使うのではなく、`02_exam_check_item_spec_v2_0_0.md` へ制度チェック用に整理しているため。
- 法定健診側と特定健診側では業務上の役割が異なり、同じ `△` 表記でも `required_flag` 付与方針を分ける必要があるため。
- `9E160` は `exam_item_master.xml_method_code` を持たず、namecode による判定対象となるため。

### 実装引き継ぎ
- 本決定を `03_decisions.md` へ同期後、Phase7実装では決定事項として扱う。
- Phase5 seed（`0010_dev_phr__exam_item_groups_v2_2026.sql`）を正とし、法定健診側の `△` 項目は `required_flag = 1` として反映する。
- 特定健診側の `△` は本決定の対象外とし、実装時に独自判断で `required_flag = 1` へ変更しない。
- `☆` 項目は `required_flag = 0` を維持する。
- `03_decisions.md` に同期済みの決定事項は再協議せず、不足がある場合のみ不足入力として報告する。

### 次回検討
- 法定健診側 `△` 項目のseed反映状況を確認する。
- 特定健診側 `△` の扱いを別途整理する。
- 条件付き必須をルールマスタで表現する方式を協議し、決定後に `03_decisions.md` へ反映する。

---

## DH-20260713-01 / 2026-07-13 03:30 JST

### テーマ
法定健診 `check_result` の判定ルール整理方法と実装手順の見直し

### 背景
Phase7の制度チェック基盤は動作しており、XML取込、検査値展開、加入者突合、`exam_check_results` 生成、`xml_ledger.check_status` / `check_reason` 更新まで実行できることを確認している。

一方、法定健診項目はすべて同じ存在判定ルールではなく、単純な値有無、同一性コード内の優先候補、複数項目の組合せ、算出、代替、条件付き必須などが混在している。

現行のルール整理では、値の探し方と直接値がない場合の補完方法が横並びになっており、項目ごとの判定根拠やNG時の切り分けが分かりにくい状態となっている。そのため、スクリプト修正を先行せず、法定健診項目ごとの判定ルールを階層化して棚卸しした上で、処理構造を見直す必要があると判断した。

### 議論
- 先にスクリプトを修正すると、未整理の制度仕様がそのままコードへ固定されるため、まず法定健診ルールを項目単位で棚卸しする。
- 対象は `02_exam_check_item_spec_v2_0_0.md` の法定健診側が `○` または `△` の項目とし、特定健診側の判定は混在させない。
- 現行の `presence_value_mode` では、直接値の探し方と直接値がない場合の補完方法が同列に扱われているため、以下の3階層へ分けて整理する。
...
#### B. メイン判定
対象項目の直接値をどのように探し、成立と判定するかを定義する。

- `SIMPLE_ANY`
  - 候補のうち有効値が1件以上あれば成立する。
- `PRIORITY_ANY`
  - 優先順位順に候補を確認し、最初に成立した値を採用する。
- `ALL_REQUIRED`
  - 指定された候補がすべてそろった場合に成立する。
- `GROUP_ANY`
  - 複数の構成パターンのうち、いずれか1組が成立すれば充足とする。

#### C. 不成立時の補完
メイン判定で成立しなかった場合に、次に行う処理を定義する。

- `NONE`
  - 補完を行わず `MISSING` とする。
- `CALCULATE`
  - 他項目から算出できる場合に `CALCULATED` とする。
- `ALTERNATIVE`
  - 別項目で制度上の要件を満たせる場合に `ALTERNATIVE` とする。

- `CONDITIONAL` は直接値の探し方や補完方法と同列ではなく、対象者に当該項目が必要かを決める上位判定として扱う。
- `CALCULATE` / `ALTERNATIVE` はメイン判定ではなく、直接値で成立しなかった後の補完処理として扱う。
- 例として、以下のように整理できる。
  - 身長: `ALWAYS / SIMPLE_ANY / NONE`
  - BMI: `ALWAYS / SIMPLE_ANY / CALCULATE`
  - 喀痰: `CONDITIONAL / SIMPLE_ANY / NONE`
- 腹囲、内臓脂肪面積、血圧、視力、聴力、胸部X線などは、候補の優先順位や組合せ、代替方向を改めて確認する必要がある。
- 未確定事項は推測で埋めず、「要確認」として一覧へ残す。

### 現時点の考え
法定健診 `check_result` の正確性を高めるには、項目ごとの個別ルールをそのままスクリプトへ書き足すのではなく、まず「適用」「メイン判定」「不成立時の補完」の3階層で制度ルールを整理する必要がある。

ルール一覧が固まった後に、その分類へ沿ってスクリプト構造とルールマスタを見直す。初期案の抽出・分類はCodexに行わせるが、制度上の最終判断は人が項目単位で確認する。

### 決定事項
- 次の作業はスクリプト改修ではなく、法定健診項目ルール一覧の作成とする。
- 対象は法定健診側が `○` または `△` の項目とし、特定健診側は今回の整理対象へ含めない。
- 各項目は以下の3階層で整理する。
  - 適用ルール: `ALWAYS` / `OPTIONAL` / `CONDITIONAL`
  - メイン判定: `SIMPLE_ANY` / `PRIORITY_ANY` / `ALL_REQUIRED` / `GROUP_ANY`
  - 不成立時の補完: `NONE` / `CALCULATE` / `ALTERNATIVE`
- 判定ルール一覧には、少なくとも以下を保持する。
  - `identity_item_code`
  - 項目名
  - 適用ルール
  - メイン判定
  - 候補 `namecode` / `method_code`
  - 優先順位または成立に必要な組合せ
  - 補完方法
  - 補完元
  - 未達時のstatus
  - 要確認事項
- 未確定項目は推測で決めず、「要確認」のまま残す。
- 法定健診ルール一覧が確定するまで、`03_check_exam_results.py` の大規模な構造変更は行わない。
- ルール一覧確定後、`xml_ledger_id` 単位で1人分の検査値を一括取得し、スクリプト内の評価コンテキストを用いて法定項目を順番に処理する構造を検討する。

### 保留事項
- 腹囲 `9N016` の実測・自己測定・自己申告の優先順位と採用条件。
- 内臓脂肪面積 `9N021` と腹囲 `9N016` の正式な代替方向。
- 血圧の「その他・1回目・2回目」の採用優先順位。
- 視力・聴力における左右・周波数・矯正値の成立条件。
- 胸部X線のmethod違いをどの単位で成立とするか。
- 貧血、心電図、喀痰等の条件付き必須を判定する正式条件。
- 現行 `presence_value_mode` / `rule_code` / `rule_source_*` を3階層ルールへどう移行するか。

### 根拠
- Phase7実データ確認で、単純な `required_flag` と値有無だけでは法定健診ルールを正確に表現できない項目が複数確認されたため。
- 値の探し方と補完処理を同列に扱うと、判定順序とNG原因の切り分けが不明瞭になるため。
- 先に制度ルールを分類してから実装構造を決めた方が、誤判定・個別ハードコード・後戻りを減らせるため。
- 1人分の検査値を評価コンテキストとして保持すれば、同一性コード内候補、算出元、代替元を同じ処理単位で参照できるため。

### 実装引き継ぎ
- まず現在の仕様表・seed・`exam_item_master` から法定健診項目を抽出し、ルール一覧の初期案を作成する。
- 初期案は単純な項目から順に確認する。
  1. 単純存在型
  2. 同一性コード内の複数候補型
  3. 複数namecodeの組合せ型
  4. 算出・代替型
  5. 条件付き型
- Codexはルール候補と根拠を整理するが、不明点を推測で確定しない。
- ルール一覧確定後に、以下の処理構造を設計する。
  - `xml_ledger_id` を1件取得
  - 対象者の `exam_item_values` を一括取得
  - `namecode` / `identity_item_code` / `method_code` 別の評価コンテキストをスクリプト内に構築
  - 適用判定
  - メイン判定
  - 不成立時の補完
  - status / reason確定
  - 1人分の結果をまとめて保存
- ルール一覧確定前に、既存スクリプトへ新しい個別ルールを追加しない。

### 次回検討
- 法定健診対象項目のルール一覧初期案を作成する。
- 単純存在型から項目単位レビューを開始する。
- ルール一覧のレビュー結果をもとに、現行seed・DDL・スクリプトとの差分を整理する。
- 3階層ルールへ移行するためのDB・スクリプト構造を決定する。

---

## DH-20260716-01 / 2026-07-16 18:00 JST

### テーマ
Phase7 法定健診 check_result の判定構造および実装方針整理

### 背景
法定健診（労働安全衛生規則第44条）の判定ルール整理を進める中で、項目ごとのルールだけではなく、スクリプト全体の責務・処理単位・共通化方針を先に整理する必要があった。

また、判定ルールとDB設計を同時に進めると議論が複雑化するため、まずは処理構造を確定し、その後DB設計へ反映する方針とした。

### 議論
- 判定処理は対象者1人単位で実施する。
- 判定開始時に、則44判定で必要となる `require_namecodes` の健診結果を一括取得する。
- 判定中はDBへ再問い合わせせず、取得した値のみを利用する。
- 判定主体は「法令項目詳細No」とし、identity・method・namecode は法令項目詳細Noを判定するための構成要素として扱う。
- スクリプト構造を三層（上位・中間・下位）に分離する。
- 共通化できる処理は ANY・PRIORITY・ALL・FINDING 等の最小単位に限定する。
- 血糖・胸部X線・腹囲・視力・聴力など、日本語ルールをそのまま表現した方が分かりやすい項目は、法令項目詳細Noごとの判定関数へ実装する。
- 判定結果は `CheckResult` として返却し、`status`・`reason` を共通形式で扱う。
- `reason` は実装者向けではなく、健診機関への確認にも利用できる日本語を基本とする。
- 詳細な設計内容は `31_phase7_legal_check_redesign.md` に整理する。

### 現時点の考え
Phase7では、法定健診判定をルールエンジンとして汎用化するのではなく、「法令項目詳細No」を中心とした構造とする。

共通化は最小限の判定パターンに留め、日本語仕様を読み替えた処理は法令項目詳細Noごとの判定関数へ実装することで、制度仕様との対応関係を明確にする。

DB構造は本処理構造を表現できる形として後続で設計する。

### 決定事項
- 判定処理は対象者1人単位で実施する。
- 判定開始時に `require_namecodes` の値を一括取得し、判定中はDBへ再問い合わせしない。
- 判定主体は法令項目詳細Noとする。
- スクリプトは以下の三層構造とする。
  - 上位：対象者単位の処理制御・値取得・結果保存
  - 中間：法令項目詳細Noごとの判定関数
  - 下位：ANY・PRIORITY・ALL・FINDING等の共通判定lib
- 共通化は最小単位の判定処理のみとし、制度固有ロジックは法令項目詳細Noごとの判定関数へ保持する。
- 判定結果は `CheckResult` として返却し、`status`・`reason` を共通形式で扱う。
- `reason` は日本語で保持する。
- 詳細な設計内容は `31_phase7_legal_check_redesign.md` を正本として管理する。
- DB設計は本方針確定後に実施する。
...
### 根拠
- 法令項目詳細No単位で実装することで、制度仕様と実装を一対一で対応付けられるため。
- 判定開始時に必要値を一括取得することで、判定処理をDBから独立させ、テスト性・保守性を向上できるため。
- 共通化を最小限に留めることで、日本語仕様をそのままコードへ反映しやすくし、制度変更時の影響範囲を限定できるため。

### 実装引き継ぎ
- 本DHで決定した内容は `03_decisions.md` へ同期後、実装時の前提として扱う。
- 実装者は `31_phase7_legal_check_redesign.md` を設計書として参照し、処理構造・責務・判定方針に従って実装する。
- `03_decisions.md` に同期済みの内容は再協議せず、不足または制度上の未確定事項のみ不足入力として報告する。
- Excel（労安法_一般健康診断_項目対応表.xlsx）は法令項目詳細Noごとの判定ルール整理資料として利用する。

### 次回検討
- Excelの法令項目詳細Noごとの判定ルール整理を完了する。
- 共通判定（ANY・PRIORITY・ALL・FINDING）のインターフェースを確定する。
- 法令項目詳細Noごとの判定関数一覧を整理する。
- 本方針を基にDB構造・DDL・判定結果テーブル設計を開始する。
- 決定事項は `03_decisions.md` へ同期する。

---
## DH-20260717-01 / 2026-07-17 13:21 JST

### テーマ
健診結果受領チェックシステム Resultチェック機能 改修方針整理

### 背景
SHG Resultチェック機能について、個別の場当たり的な修正ではなく、
今後も保守しやすい構成へ整理しながら改修を進める方針を確認した。

### 決定事項

- `check_shg_result_xml.py` は全体の制御（オーケストレーター）に専念させる。
- 個別仕様への対応は `script_lib` 配下へ切り出して実装する。
- 呼び出し側は必要な情報のみ渡し、修正ロジック側で判定・修正を完結させる。
- 修正処理には必要最小限の値のみ引数として渡す。
- 今回の Outcome Total Point 修正では保険者番号による条件分岐が必要となったため、
  `insurer_number` を追加引数として渡す構成とした。
- 修正ロジックは `outcome_point_block_fix.py` に集約し、
  `check_shg_result_xml.py` 側には処理内容を書かない。
- 今後も仕様追加が発生した場合は同様に `script_lib` 側へ機能追加する。

### 実装内容

- `check_shg_result_xml.py`
  - `apply_outcome_total_point_block_fix()` 呼び出し時に
    `insurer_number=str(basic.get("insurer") or "")`
    を追加。

- `script_lib/outcome_point_block_fix.py`
  - `insurer_number` を受け取れるよう引数を追加。
  - 保険者番号を利用した条件判定を実装。
...
- `python3 -m py_compile`
  正常終了。
- 呼び出し元は `check_shg_result_xml.py` の1箇所のみ変更。
- 既存修正との競合なし。

### 今後の方針

Resultチェックは、個別案件ごとの例外処理が増加することを前提とし、
例外ロジックは `script_lib` に分離して蓄積する。

メイン処理は読みやすさを維持し、
仕様変更はライブラリ単位で追加・保守できる構成を継続する。

---

## DH-20260717-02 / 2026-07-17 15:25 JST

### テーマ
法定健診判定基盤の4層構造および共通判定ライブラリ設計方針の整理

### 背景

則44判定仕様（32）の整理が進み、実装前に判定基盤の責務分割を明確にする必要が生じた。

制度判定ロジックは今後、特定健診等へ拡張することを想定しているため、DBアクセス・判定処理・共通処理を分離した構造とするかを協議した。

### 議論

- 判定基盤を4層構造とする案を協議した。
- 第1層は制度ごとに必要なnamecode一覧を取得する責務とする。
- 第2層は対象者単位で必要namecodeの健診結果を取得し、判定用データを生成する責務とする。
- 第3層は法令項目詳細Noごとの判定関数を配置し、制度仕様を実装する責務とする。
- 第4層はANY・FINDING・PRIORITY等の最小粒度の共通判定関数のみを配置する方針とした。
- 第2層はDBアクセスのみを担当し、制度判定を持たない。
- ST項目は文字列そのものではなく、判定に必要な状態へ正規化して返却する方針を協議した。
- STは将来拡張可能な構造とし、現時点では決定済みの分類のみ返却する方針とした。
- 共通判定ライブラリの詳細設計は別仕様書として整理する方針とした。

### 現時点の考え

制度判定は法令項目詳細Noごとに実装し、共通ライブラリは最小粒度の判定処理のみを提供する。

DB取得処理と制度判定を分離することで、則44・特定健診など複数制度で同一基盤を利用できる構造とする。

### 決定事項

- 判定基盤は4層構造とする。
- 第1層は必要namecode一覧取得とする。
- 第2層は対象者単位の健診結果取得と判定用データ生成を担当する。
- 第3層は法令項目詳細No単位の判定関数とする。
- 第4層は最小粒度の共通判定ライブラリとする。
- 第2層は制度判定を行わない。
- ST型は文字列をそのまま返さず、判定用状態へ正規化して返却する。
- STで返却する状態は現時点では以下とする。
  - NOT_FOUND
  - NULL
  - EMPTY
  - PRESENT
...
- DBアクセスと制度判定を分離することで責務を明確化できるため。
- 共通判定を最小粒度へ限定することで制度固有ロジックの肥大化を防止できるため。
- ST文字列を分類して返却することで、所見有無未登録など施設差異への対応余地を確保できるため。

### 実装引き継ぎ

- 33_check_framework_design.md に共通判定基盤仕様を整理する。
- 03_decisions.md へ同期後は判定基盤構造を決定事項として扱う。
- 実装時は33のインターフェース仕様を優先し、不足がある場合のみ不足入力として報告する。

### 次回検討

- 33_check_framework_design.md の作成。
- 第4層共通判定ライブラリの関数一覧とインターフェース仕様の確定。
- ValueMapおよびCheckResultの詳細仕様を決定する。
