...
2026-08-18時点では、ユーザー管理は試走しながら必要機能を追加しているため、運用画面としては概念と表示の整理が必要である。

今後の議題:

- 承認状態と利用可否を画面上で分離する。`承認待ち/承認済み/却下` は申請・承認フロー、`有効/無効` はログイン可否として扱う。
- アカウント一覧では、直接操作を最小限にし、詳細・編集画面へ寄せる。状態変更、ロール変更、作業権限、許可IP、パスワード初期化は詳細画面でまとめて確認・更新できるようにする。
- 本人からの登録申請と管理者によるアカウント発行を別フローとして明確化する。本人申請は `承認待ち + 閲覧者`、管理者発行は `承認済み + 指定ロール` を基本とする。
- ロールと個人別作業権限の役割を画面上で明確化する。ロールは基本責務、個人別作業権限は出力リスト、XML出力、HIAアップロード等の担当ON/OFFとして見せる。
- 最後の管理者を無効化・権限剥奪できない制御は維持しつつ、ボタン無効だけではなく理由を表示する。
- IP制限は、申請時IP、許可IP、最終ログインIPを区別して表示する。許可IP未設定時は「IP制限なし」と明示する。
- 管理カテゴリは、業務管理とシステム管理を分ける。業務管理は編集者にも許可できるが、アカウント、権限、セキュリティ、監査ログは管理者のみを基本とする。
- 個人情報を含む画面閲覧、ダウンロード、正式出力、アップロード記帳は監査ログ対象として継続する。

初期の整理方針:

- DB構造は大きく変更せず、まず画面表示、文言、操作導線を整理する。
- 必要になった場合のみ、migrationを追加してDB構造を拡張する。既に適用済みのmigrationは変更しない。
