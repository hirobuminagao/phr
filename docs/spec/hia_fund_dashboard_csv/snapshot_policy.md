...
# 10. イベント連携ポリシー（HIAダッシュボード）

HIAダッシュボードは「現在状態」を表すスナップショットであり、
同一人物の状態は run ごとに上書きされる。

特に年度切替時には、次のような状態リセットが発生する可能性がある。

- 受診済み → 未予約
- 結果登録済み → 未予約

このため、イベント（event）に対する状態判定を
`hia_dashboard_status` の最新値参照で行ってはならない。

## 基本方針

イベントに必要な情報は、参照ではなく
観測時点の事実として `event_instance` に記帳する。
...
`run_id` は「どのスナップショット時点の状態か」を示す証跡である。

これにより、

- 過去年度の状態が上書きされた場合でも
- 特定の run_id を起点として状態を再現することが可能となる
...
- event_instance（run_id付き観測データ）
- hia_dashboard_status_history（差分履歴）

## 結論

HIAダッシュボード由来のデータは

- 最新状態として参照するもの（status）
- 観測事実として記帳するもの（event_instance）
