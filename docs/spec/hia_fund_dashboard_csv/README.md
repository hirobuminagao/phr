...
HIAダッシュボードCSVは健診結果の正本ではなく補助状態なので、HOMEの「ユーティリティ」から専用画面へ遷移する。

画面ではeventを選択し、eventに登録された保険者番号を更新対象として表示する。eventは保険者番号の選択ミスを防ぐ入口であり、ダッシュボード状態の更新範囲はeventではなく保険者番号全体とする。同じ保険者番号を持つ複数eventは同じ更新対象になる。

画面の取込方式は次の2種類とする。

- 全件更新: 保険者全体のCSVとして取り込み、CSVに存在しない既存の有効行を非アクティブ化する。
- CSV掲載者のみ更新: アップロードCSVに存在する人だけを追加・更新し、CSVに存在しない既存行は変更しない。

本実行の前にdry-runによる事前確認を選択できるようにする。アップロードファイルは、選択eventから取得した8桁保険者番号を名前に持つ一時フォルダへ保存し、既存スクリプトへ `--input` で渡す。処理終了後は一時ファイルを破棄する。

画面には処理件数とスクリプト出力を表示し、実行記録は既存どおり `work_other.etl_runs` / `work_other.etl_errors` に残す。DB構造は追加しない。
