...
- 定期健診の制度チェックは、法定健診チェックと特定健診チェックを基本的に両方実行する。事業所向けには法定健診、健保/HIA/支払基金向けには特定健診が必要になるため、通常運用で片方だけを標準にはしない。
- 法律・制度に根拠があり、全体共通で継続的に確認するチェックは `exam_check_results` の横持ち項目として扱う。対象は、法定健診チェック（まずは労安則44。後続で他法定区分を追加可能）と特定健診チェックまでを基本範囲とする。
- 横持ちにする理由は、一覧、集計、出力可否判定で毎回高速に参照するためである。制度改正により全体ルールが変わる場合は、migration/seed/共通チェック処理の更新で追従する。
...
- 特定健診チェック結果は、既存の `exam_check_results.specific_check_result` / `specific_reason_summary` を使う。元々特定健診チェック用に想定していた枠であるため、新しい結果カラムを増やす前にこの枠を正として整備する。
...
- 任意チェックの例は、特定の健保が独自に確認したい検査項目、事業所別の任意検査有無、施設別の追加確認、納品前の個別要求である。これらは制度チェックとは責務が違うため、可変ルール/縦持ち結果として設計する。
...
- CSVに対応項目がない、または対応項目の値がNULLの場合は、健診年度末日の満年齢判定で不足コードを補完する。
- 満年齢40～74歳は `health_exam_report_category = 10`, `program_code = 010`、それ以外は `health_exam_report_category = 40`, `program_code = 990` とする。
- `event.age_reference_date` は予約時のコース分け等に使う運用日付であり、特定健診対象判定および報告区分/プログラムコード補完の年齢基準日としては使わない。
...
- 条件付き出力OKではXML出力用の架空entryは作らない。一方で、画面上の記帳単位を統一するため、不足そのものは `exam_case_check_review_items` にcase用確認項目として作成する。
- `exam_case_check_review_items` は値の存在ではなく「このcaseでこの業務チェック項目が不足している」という判断対象を表す。XML出力時はentry化しない。
- 理由ありOKはcase単位の一括許可ではなく、不足しているcaseチェック確認項目単位の許可とする。case内にMISSINGが3項目ある場合は、3つの `exam_case_check_review_items` 全てに `APPROVED_WITH_REASON`、承認者、承認日時、理由auditが必要であり、1項目でも未記帳ならcaseは出力不可のままとする。
- 同じ理由を複数項目へ一括適用する画面操作は許可する。ただしDB上は対象の `exam_case_check_review_items` を1行ずつ更新し、`exam_case_check_review_item_audit_logs` も1行ずつ作成する。まとめ操作の便利さと、項目ごとの通過責任は分けて扱う。
...
- `03_04_check_exam_export_cases.py` によるcase再チェックは、`exam_check_results` を再作成し、`exam_export_cases.check_status` / `check_reason` と出力可否summaryを再計算する。人手判断の正はcase直持ちではなく、`exam_case_check_review_items` に置く。
- `03_01_build_exam_export_cases.py` のcase再作成も、同じcaseキーで既存行を更新する場合は既存case確認項目の `review_status` を保持する。そのため同一caseのままなら、全チェックを再実行しても理由ありOKや確認待ちは保持される。
- ただし、加入者、健診日、健診機関、保険者番号などcase同一性キーが変わり、別caseとして新規作成された場合は、既存のcase確認項目に対する人手判断は自動移行しない。理由ありOKは人が確認したcase内の業務チェックID単位の判断として扱う。
...
- 確認事項の画面入口はcaseまたはledgerとする。存在するsource値や健保/HIAからnamecode指定で来る個別指摘は `exam_item_values`、存在しない法定/特定健診の不足は `exam_case_check_review_items` を物理単位にする。
- 存在するsource値は `ledger_type = 'EXAM'` の `exam_item_values` に保持する。存在しない不足項目は、case再チェック時に `exam_case_check_review_items` として作成する。
...
- 特定健診チェックで `specific_reason_summary` に出た `NOT_FOUND`、`NULL`、`EMPTY`、`CODE_VALUE_MISSING`、`TEXT_VALUE_MISSING` も `exam_case_check_review_items` として作成する。特定健診項目は法定健診より理由ありOKが多くなる可能性があるが、HIA/XML受付上の不足確認対象として同じ記帳導線に載せる。
...
- 手動出力許可後も `exam_check_results` と結合出力用caseの `check_status` は書き換えない。`exam_case_check_review_items` は該当entryとしてXMLへ出力しない。
- `PRIMARY` と `SUPPLEMENT` の両方に同一namecodeが存在し、どちらもVALIDだが意味が異なる場合は、MISSINGではなく採用優先ルールまたは健診機関確認対象として扱う。例として、XML側の医師の診断（判定）にメタボリックシンドローム判定の口語文が入り、CSV側に医師コメントらしい長文があるケースは、不足確認項目ではなく precedence rule / 確認ルールで採用元を制御する。
...
- 出力リストの初期DDL候補名は `ops_xml_export_lists` / `ops_xml_export_list_cases` とする。`run_id` という名前はETL実行履歴と混同するため避ける。
- `etl_runs` は出力処理を実際に走らせた実行ログとして使い続ける。出力リストは「誰を今回出す候補にしたか」、`xml_export_zips` / `xml_export_members` は「実際にどのZIP・個人XMLを出したか」を表す。
- HIA XML出力リスト画面は、箱作成前と作成後で表示を分ける。箱作成前は状態を `未作成` とし、リスト名、対象event、提出日、出力番号、作成時に含める対象だけを扱う。箱作成後は `DRAFT` となり、作成時の初期追加ブロックは非表示にする。
- 箱作成時に含める対象は `READY` と `理由ありOK` の2つをチェックで選ぶ。チェック対象を入れて作る場合は「選択した人を追加して作成」、箱だけ作成して検索へ進む場合は「リスト作成して人を追加」とする。
