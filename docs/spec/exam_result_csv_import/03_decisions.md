...
- 特定健診チェックも則44と同じく「制度detail code -> namecode候補 -> OK/MISSING/INVALID」の形へ寄せる。則44の `4401001001` などと衝突しない特定健診用detail codeを定義し、必要namecode群は `dev_phr.exam_item_group_members` 等のルールマスタで管理する。
