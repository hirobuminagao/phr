...
XML側に該当する `namecode + occurrence_no` が存在しない場合は、CSV側の正規化済み値を不足補完として採用する。
採用済み整値の基本順序は、XMLで取れる値を優先し、XMLにない値をCSVで補い、後続では紙入力や手修正値で補う形とする。
CSVはXMLの全面置換ではなく、XML不足項目を埋めるための補完sourceとして扱う。
...
- 基本情報補正はcase単位の現在値と履歴で初期実装する。source ledgerへの逆参照を追加で持つかは、加入者突合修正や再取込時の再反映フローを見て後続判断する。
