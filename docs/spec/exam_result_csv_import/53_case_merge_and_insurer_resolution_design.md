...
同一チェック項目のreviewが両caseに存在しても、双方がチェック処理由来の自動状態だけなら統合を許可する。統合先のreview IDと現在状態を残し、統合元の監査ログを統合先へ移して、統合後の再チェックで採用値から状態を再判定する。理由ありOK、除外、再提出待ち、note、操作者または画面操作の監査履歴を含む場合は、人手判断の競合として停止する。
