...
- 健診機関サマリー詳細では、source NG理由をXML/CSVだけに限定し、補正用の紙・手入力sourceを原本NG理由へ混在させない。
- 健診機関サマリー詳細では、聴力4項目の正常/異常ledger数、異常割合、変換後CDと想定raw値を確認できる。
- 健診機関サマリー詳細では、特定健診チェックの未解消項目をnamecode、理由、case数、確認状態ごとに集計できる。
...
- HIAアップロード記帳、CSVマッピング管理、紙健診入力は初期運用版を実装済みであり、運用で得た例外パターンを段階的に追加する。
- 個人case詳細、基本情報補正、加入者突合NG修正は初期版実装済み。運用しながら候補表示、警告、再反映導線を調整する。
- CSVマッピングテンプレートは、健診機関選択、基準ヘッダー保存、シンプル1:1、複数列結合、画面作成ルールの編集・削除、有効/無効を扱う。
- CSVマッピング編集は、既存の取り込み先・CSVヘッダー選択を流用した`条件分岐`の初期版を実装済み。別モーダルで空欄時の処理、比較条件、固定登録値を設定し、同一グループの複数ruleとして作成・再編集・削除する。詳細は `52_csv_mapping_conditional_branch_editor_design.md` を正とする。
- CSVヘッダープレビューはキーワード検索と複数一致の前後移動に対応する。
- CSVマッピングテンプレートでは、選択施設の特定健診対象caseを母数にMISSING 100%の項目をtarget追加候補として表示し、既存マッピング状態とXML受領件数を併記する。
- CSVマッピング詳細から、設定済みテンプレートへ実CSVデータを仮適用して変換結果を確認する初期チェック機能を実装済み。小禄の聴力で発生したCD変換の意味・値の取り違えのように、ルール自体は動作していても変換結果の分布が不自然な問題を、本取込前に発見することを目的とする。
  - 導線はCSVマッピング設定詳細に置き、表示中の保存済みテンプレートと保存済みルールを使用する。無効テンプレートでも実行可能とし、無効から有効へ変更してよいか確認する用途を重視する。無効ルールと未保存の編集内容は対象外とする。
  - アップロードしたCSVから、選択したテンプレートに設定済みの列だけを読み込む。通常取込、ledger作成、case更新は行わないdry-runとする。既存の変換処理にDB参照が必要な場合はDBを使用してよいが、アップロードファイルと実値は処理完了時および異常終了時に破棄し、永続化しない。
  - 文字コードはUTF-8、UTF-8 BOM、CP932等を可能な限り自動判定する。ヘッダー行とデータ開始行はテンプレート設定を使用し、基準ヘッダーと一致しない場合は処理開始前に差分警告を表示する。
  - 集計対象は先頭最大10,000行とし、CSVがそれを超える場合は処理対象件数と切り捨て件数を明示する。実値一覧は先頭100件に加えてエラー行を表示する。
  - 項目別のリストヘッダーには、処理対象行数、値あり件数・率、空欄件数・率、変換成功件数・率、変換失敗件数・率、適用ルールを表示する。各割合の分母は実際に処理した行数とし、15,000行中10,000行を処理した場合は10,000行を分母にする。
  - `CD`・`CO`は、変換後コード、表示名、件数、全処理対象行に対する割合を表示し、raw値と変換後コードの対応を確認できるようにする。`CO`はコード単位とコード組み合わせ単位の両方を集計する。
  - `PQ`は値あり率、変換・validation結果、実値をプレビューする。
  - `ST`は優先度を下げ、初期実装では詳細集計を省略してよい。
  - 1:n結合は元列ごとのraw値と最終結合値、条件付き固定値は成立した条件と出力値を表示する。ルール不一致行と複数ルールが競合した行も確認対象にする。
  - エラーは、ヘッダー不足、条件不一致、値型不正、normalize失敗、validation失敗、複数ルール競合に区分し、可能な限り通常importと同じ判定処理を共用する。
  - 集計の下には行単位の実値一覧を置き、CSV行番号、raw値、変換後の値、適用ルール、エラーを確認できるようにする。
  - 検証履歴と集計結果は保存しない。CSVアップロード時のヘッダー・コンテキスト・データ行プレビューですでに実データを扱うため、専用権限は追加せず、CSVマッピング編集権限を持つユーザーだけが実データチェックを開始・閲覧できるようにする。
- 加入者情報確認ユーティリティは参照専用の初期版を実装済みで、完全表示・マスク表示・非表示の権限を分ける。
- 業務工数試算画面はユーティリティに配置し、想定値とDB実数、法定NG率、1施設平均case数、増員目安を表示する。
- 受診月単位の全体NG報告は初期版を実装済み。現在のDB状態から全体・施設別・項目別NGとCSV/手入力の採用case・項目数を参照集計する。厳密な救済再判定と月次確定スナップショットは `49_monthly_ng_and_rescue_reporting_design.md` の後続フェーズとする。
...
- 個人case詳細の初期版では、case一覧から詳細へ遷移し、構成source、採用値、case単位チェック履歴、MISSING placeholderを確認できる。placeholderはitem_value単位で確認待ち、理由ありOK、再提出待ち等を保存し、auditへ履歴を残す。
- 理由ありOKの保存後も機械チェック結果は書き換えない。caseへの反映は「健診結果処理実行」の step5〜7 再実行で行う。
...
### 13. Handoff, Facility Notes, Notifications, and Change Requests

運用上の確認事項を、健診機関、alias、event、case、ledger、CSVマッピング等へ関連付け、起票者・担当者・確認先・状態・期限を追跡する申し送り機能を将来候補とする。

- 健診機関全体とalias固有の恒久注意事項を分ける。
- HOME・共通ヘッダーにベルとユーザー別未読件数を表示する。
- 既読状態と業務対応状態は分離する。
- case ID、健診機関ID、alias ID等の複数関連リンクを持つ。
- 改修要望も登録可能にするが、業務申し送りと同一機能・同一テーブルにするかは未決定とする。

詳細は `50_handoff_notice_and_change_request_backlog.md` を参照する。
