...
入力内容を `manual_exam_entry_drafts` / `manual_exam_entry_draft_values` に保存する。

状態:

- 保存直後は `DRAFT`
- 必須情報が揃っている場合は `READY` へ更新可能

初期実装では、保存ボタン押下時に以下を最低限チェックする。
...
4. 仮登録削除
5. 本データ反映処理
6. `03_00` が手入力sourceを対象にすることを確認/修正
7. `03_01` が手入力sourceからcaseを作ることを確認/修正
8. `03_02` が手入力sourceを採用値候補に入れることを確認/修正
9. `03_04` で法定チェック・特定健診チェックへ反映
10. 画面上で本反映後にstep再実行を案内
