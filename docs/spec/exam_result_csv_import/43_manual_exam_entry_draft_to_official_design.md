...
手入力の保険者番号が全桁0の場合、原本値をcase結合キーとして使用しない。出力用補正値、紐付け済みcaseの補正値、event保険者番号の順で有効値を解決し、正式ledger化、case完全一致確認、case再作成で同じ値を使用する。これにより、case側を補正した後に手入力sourceだけが別caseへ分離することを防ぐ。
...
- 健診結果仮登録リストでは、同一event内の仮登録を対象に同一人物候補を表示・絞り込みできる。
- 判定キーは `subscriber_id`、HIA加入者ID、person ID、保険者・記号・番号・枝番・生年月日の組み合わせ、カナ氏名・生年月日の組み合わせを使用する。
- いずれかのキーが一致する仮登録を同じ候補グループとして扱う。自動削除や自動統合は行わず、担当者がdraft ID・状態を確認するための補助情報とする。
- 重複候補件数とグループ数をサマリー表示し、「同一人物の重複候補のみ」で絞り込める。
- HIA加入者IDとcase IDは部分一致で絞り込み、重複候補条件と併用できる。
