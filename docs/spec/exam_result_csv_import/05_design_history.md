...
- `exam_facilities.address` は現時点では住所文字列を1カラムで保持しており、都道府県・市区町村・町域などへ分解していない。
- 管理画面の都道府県絞り込みは、暫定的に `address LIKE '<都道府県>%'` で行う。支払基金CSV由来の住所表記では実用できるが、住所表記揺れや将来の地域別集計・施設マスタ整備には弱い。
- 今後のマスタ整備課題として、`exam_facilities` または関連住所マスタに `prefecture`、`city`、`street_address` 相当を分離保持するか、住所正規化済み検索用カラムを追加することを検討する。
...
- 初版で確認する特定健診固有項目は、メタボ判定、保健指導レベル、医師の診断（判定）、医師名、服薬1-3、既往歴1-3、貧血、喫煙、20歳からの体重変化、運動習慣、身体活動、歩行速度、食べ方2、食習慣、睡眠、特定保健指導の受診歴とする。
- `9N141000000000011` 採血時間は特定健診XML上の重要項目だが、法定44の血糖チェック側でも同じ値を条件として使うため、特定健診固有detail横持ちからは除外する。
...
## DH-20260824-01 / 2026-08-24 JST

### テーマ

健診結果手入力の仮登録チェック結果保存先

### 背景

- 手入力は、紙健診、XML/CSV不足補足、再提出補正に使う。
- 入力後すぐに正式な `exam_ledgers` / `exam_item_values` を作ると、入力途中や確認前の値がcase作成や出力可否へ混ざる。
- そのため、手入力はまず `manual_exam_entry_drafts` / `manual_exam_entry_draft_values` に仮登録として保存する方針にした。
- 一方で、作業者は本データ反映前に「この入力で法定チェック・特定健診チェックが足りるか」を確認したい。
- 正式な `exam_check_results` は、正式なsourceやcaseの出力可否判断に使うため、仮登録中の参考チェック結果を混ぜると運用上危険である。

### 決定

- 仮登録中の法定チェック・特定健診チェック結果は、正式な `exam_check_results` へ保存しない。
- 仮登録チェック結果は、専用の `manual_exam_entry_draft_check_results` に保存する。
- チェックロジックは正式チェックと共通化する。
- 保存先だけを、正式チェックでは `exam_check_results`、仮登録チェックでは `manual_exam_entry_draft_check_results` に分ける。
- 仮登録チェックは本データ反映前の参考チェックであり、XML出力可否、出力リスト作成、HIAアップロード可否の正にはしない。
- 正式な出力可否は、本データ反映後に `03_00`〜`03_04` を実行して作成される正式な `exam_check_results` を正とする。
- 仮登録チェックで判定不能となる場合は、処理を落とさず、判定不能理由を保存して画面へ表示する。
- 仮登録チェックの実行入口は `健診結果仮登録リスト` に置く。
- `健診結果処理実行` 画面へ置く場合も、`03_00`〜`03_04` の通常stepには混ぜず、仮登録チェック用の別ブロックとして扱う。
- 仮登録リストでは行単位/一括操作として実行できるようにする。

### 保守注意

- 法定チェック・特定健診チェックのルール追加時は、正式チェックと仮登録チェックの両方を影響範囲に入れる。
- 正式 `exam_check_results` の横持ちカラムを追加する場合、必要に応じて `manual_exam_entry_draft_check_results` にも同等項目を追加する。
- 画面表示、DDL、migration、チェック結果保存処理はセットで更新する。
- メンテナンス箇所は増えるが、仮登録と正式出力可否を混同しないことを優先する。

---

## DH-20260824-02 / 2026-08-24 JST

### テーマ

特定健診チェック項目別結果の横持ち化をdraft側で先行検証する

### 背景

- 特定健診は、健保/HIA/支払基金向けに継続的に確認する固定制度チェックである。
- 固定制度チェックは、一覧、集計、出力可否判定、理由確認で高速に参照するため、法定健診と同じく横持ち `status` / `reason` を正にする方が扱いやすい。
- 現行実装では、特定健診は `specific_check_result` / `specific_reason_summary` だけを保存しており、項目別の正がない。
- そのため、case確認項目 `exam_case_check_review_items` は `specific_reason_summary` をパースして作成しているが、これは作業画面へ載せるための暫定処理であり、恒久的な機械判定の正としては弱い。
- 今年度データの受領はまだ一部であり、今のうちに保存構造を直せば再チェックによる再生成で追従しやすい。
- 一方で、正式 `exam_check_results` をいきなり変更すると既存case/ledgerの確認や出力処理へ影響するため、まず本データへ混ざらないdraft側で検証したい。

### 決定

- 特定健診の項目別チェック結果は、固定detail code単位の横持ち `status` / `reason` として保持する方針に変更する。
- `specific_check_result` / `specific_reason_summary` は総合判定とsummary枠として残す。
- 法定健診チェックと重なる項目は、特定健診側に二重のdetailカラムを持たせない。
- 最初に `manual_exam_entry_draft_check_results` へ特定健診detail code別の横持ちカラムを追加し、draft参考チェックで保存する。
- draft側で、detail code、カラム名、summary生成、画面表示、確認項目作成の形を検証する。
- draft側で問題がないことを確認した後、正式 `exam_check_results` に同じ特定健診detail code別の横持ちカラムを追加する。
- 正式側へ反映した後は、`specific_reason_summary` のパースではなく、横持ち項目別結果を正として `exam_case_check_review_items` を作成する。
- 既存正式データは `exam_item_values` / `exam_export_case_values` を元に、source/caseチェックを再実行して横持ち結果を再生成する。

### 保守注意

- draft先行期間中は、draft側だけが特定健診項目別横持ちを持ち、正式側はsummary中心のままになる。この差は意図した検証期間の差分であり、後続で正式側へ横展開して解消する。
- 法定健診・特定健診のような固定制度チェックは横持ちを基本とする。
- 健保独自、事業所独自、納品先独自、外部指摘、作業状態のように可変になり得るものは、制度チェック横持ちへ混ぜず、別の柔軟な器で扱う。

---

## DH-20260825-01 / 2026-08-25 JST
...
- 現行の `健診結果処理実行` は、画面のPOSTリクエスト中に各stepを順番に実行し、完了または失敗まで画面が待つ。
- 処理対象が増えると、画面が固まって見える、ブラウザやプロキシのタイムアウトを受ける、二重クリックや連打を防ぎにくい、実行中処理の把握が難しい、という運用リスクがある。
- 既に `etl_runs` / `etl_errors` は処理ログとして使っているが、画面からの実行受付、進捗、停止要求、実行者、同一event内の排他制御を扱うには、管理画面用のジョブ状態が必要になる。

### 方針

- 今すぐ実装はしないが、後続で処理実行を非同期ジョブ化する。
- 処理実行専用ではなく、管理画面共通の非同期ジョブリストとして設計する。
- 画面操作から作成されたジョブを共通リストへ積み上げ、ワーカーが順番に処理していく方式を基本にする。
- 処理ボタン押下時は、同じ処理が共通ジョブリストに未完了状態で存在するかを先に確認する。
- 同じ処理が `queued` / `running` / `cancel_requested` 等で存在する場合は、新規追加せず「同じ処理が実行中または待機中」として実行不可にする。
- 同じ処理が存在しない場合だけ、ジョブを追加して受付完了にする。
- 実行ボタン押下時は、管理画面用のジョブを作成して即時に画面へ戻す。
- バックグラウンド処理で選択stepを実行し、現在step、開始時刻、終了時刻、実行者、状態、失敗理由、ログ参照を保存する。
- 画面には実行中ジョブ一覧、直近ジョブ一覧、ジョブ詳細ログへの導線を置く。
- 同じeventで競合する処理が実行中の場合は、サーバー側でも開始を拒否する。
- 管理者または現場管理者は、長時間止まったジョブに対して停止要求を出せるようにする。

### 実装候補

1. `admin_jobs` / `admin_job_steps` 相当の管理画面共通ジョブテーブルを追加する。
2. `/exam-processing/run` は同期実行ではなく、ジョブ作成とバックグラウンド起動だけ行う。
3. 実処理は既存のstep実行関数を再利用し、stepごとの結果をジョブ側へ保存する。
4. `etl_runs` / `etl_errors` はスクリプト実行ログの正として残し、管理画面ジョブは画面操作と進捗の親として扱う。
5. 停止は最初から強制killではなく、停止要求フラグを見てstep境界で止まる方式を優先する。
6. ジョブには `job_type` を持たせ、健診結果処理実行、仮登録チェック、HIAアップロード補助、再チェックなどを同じ一覧で扱えるようにする。
7. `queued`、`running`、`succeeded`、`failed`、`cancel_requested`、`cancelled` などの状態を持ち、積み上げたジョブの待ち順と現在処理中を画面で確認できるようにする。
8. 重複判定用に、`job_type`、`event_id`、対象ID、対象step、主要オプションから作る `dedupe_key` を持たせる。
9. `dedupe_key` が同じ未完了ジョブがある場合は、二重実行を防ぐため登録を拒否する。

### 注意点

- 非同期化しても、実行可能stepの組み合わせ制御は画面だけでなくサーバー側にも残す。
- 実行中に画面を閉じても、ジョブ状態とログを後から確認できることを必須にする。
- Docker/Windows直起動の両方で動く方式にする。外部キュー導入は初期版では必須にしない。
- 重複判定は処理種別ごとに粒度を変える。例: 健診結果処理は `event_id + step群 + dry_run + include_imported`、仮登録チェックは `draft_id` または `draft_id群`、HIAアップロード補助は `export_zip_id` または `export_list_id` を候補にする。

---

## DH-20260825-02 / 2026-08-25 JST
...
- XML取込 `scripts/from_medical/02_import_xml.py` は、暗号化された受領ファイルを処理する際に `work_other.medi_zip_passwords` からパスワードを解決している。
- 現行テーブル名は `medi_zip_passwords` だが、実務上はZIPだけに限定せず、XML取込時に必要な受領ファイルのパスワード管理として扱う。
- 現行の主な解決キーは `ZIP_SHA256`、`ZIP_NAME`、`FACILITY` で、`FACILITY` では `file_receipts.facility_code`、`submitter_facility_code`、受領フォルダ名を候補として参照する。
- パスワードが見つからない場合は `ZIP_PASSWORD_NOT_FOUND`、合わない場合は `ZIP_DECRYPT_FAILED` 等の取込エラーに残る。
- そのため、新規テーブルを先に作るより、まず既存取込処理が参照している `work_other.medi_zip_passwords` を画面から管理できるようにするのが最短である。

### 方針

- 初期実装では、既存テーブル `work_other.medi_zip_passwords` を管理対象にする。ただし画面名や説明はZIP専用に寄せず、受領ファイルパスワード管理として扱う。
- `02_import_xml.py` のパスワード解決ロジックは原則変更しない。
- 一覧ではパスワード本文を表示しない。登録有無、適用範囲の種類、対象施設/ZIP/XML、優先度、有効状態、更新日時が分かればよい。
- パスワード本文の確認は、特定の許可ユーザーと管理ユーザーのみに限定する。
- パスワード登録、変更、本文確認、無効化、削除を監査ログ対象にする。
- `ZIP_PASSWORD_NOT_FOUND` / `ZIP_DECRYPT_FAILED` 等の取込エラーから、パスワード登録または編集へ進める導線を作る。
- 削除は初期実装では行わず、無効化を基本にする。

### 初期実装の決定

- 画面名称は `受領ファイルパスワード管理` とする。
- 対象テーブルは `work_other.medi_zip_passwords` とする。
- 対象scopeは既存互換で `FACILITY` / `ZIP_NAME` / `ZIP_SHA256` を扱う。
- 一覧は本文非表示とし、登録有無と種類を確認できる表示にする。
- 本文確認は `received_file_passwords.view_secret` を持つユーザーと管理ユーザーだけに許可する。
- 登録/編集/無効化は管理カテゴリ操作権限を持つユーザーに許可する。
- 取込エラーからの導線は、処理実行画面と受領ファイル一覧の両方に置く。
- 登録後は、XML取込の再実行が必要であることを画面で案内する。
- 権限は `received_file_passwords.view`、`received_file_passwords.manage`、`received_file_passwords.view_secret` の3つに分ける。
- 初期ロールは、`ADMIN` が3権限すべて、`FIELD_MANAGER` と `EDITOR` が `view` / `manage`、`VIEWER` は全てOFFとする。
- 特定ユーザーにだけ本文確認を許可する場合は、個人別作業権限で `received_file_passwords.view_secret` を付与する。

### 画面要件

1. パスワード一覧では、`scope_type`、対象キー、パスワード有無、priority、有効状態、メモ、更新日時を表示する。
2. パスワード本文は一覧に出さず、`あり` / `なし` またはマスク表示にする。
3. 新規登録/編集はモーダルまたは別画面にし、`scope_type` に応じて入力項目を切り替える。
4. `FACILITY` では健診機関コード、submitter施設コード、受領フォルダ名を扱えるようにする。
5. 既存互換のため、キー名は当面 `ZIP_NAME` / `ZIP_SHA256` を使うが、画面表示上は受領ファイル名/受領ファイルsha256として扱い、ZIP専用に見せない。
6. 健診機関コードや受領フォルダ名は、可能であればalias/健診機関マスタ検索から選択できるようにする。
7. 取込エラー画面または処理実行結果から、受領ファイル名、sha256、施設コード、受領フォルダ名をプリセットして登録へ進める。
8. 復号失敗では既存登録が間違っている可能性が高いため、対象候補を表示して編集へ誘導する。
9. パスワード本文は `表示` と `コピー` の両方を用意し、どちらも監査ログへ残す。
10. 編集時にパスワード欄が空欄の場合は、既存パスワードを維持する。

#### 受領alias選択と複数候補試行（2026-09-09）

- 健診機関単位の登録ではコードやフォルダ名を直接入力せず、受領aliasを選択する。
- 保存時にaliasの`src_folder_raw`と紐づく健診機関コードを既存の`FACILITY`行へ記録する。
- XML取込はSHA、ZIP名、健診機関コード・受領フォルダ名の優先順で有効な候補をすべて取得する。
- 同じパスワード本文は1回だけ試し、暗号化された対象XMLをすべて読めた最初の候補を採用する。
- 候補本文や失敗したパスワードはログへ出さない。
11. `scope_type` の画面表示は、`FACILITY` を `健診機関/受領フォルダ`、`ZIP_NAME` を `受領ファイル名`、`ZIP_SHA256` を `受領ファイルSHA` とする。

### 注意点

- 既存互換で `password_text` が平文である期間でも、画面上はマスク表示を基本にする。
- 本文確認操作は権限で制御し、誰がいつ確認したかを監査ログに残す。
- 将来は暗号化保存、利用時のみ復号、閲覧権限制御、コピー可否を検討する。
- `exam_facility_id` による解決は初期実装では追加しない。alias/施設マスタ整理後に、解決キーへ追加するか検討する。
- 将来、ZIP/XML以外の暗号化受領ファイルが増える場合は、`scope_type` 名やテーブル名の見直しを検討する。初期実装では既存取込処理との互換を優先する。
