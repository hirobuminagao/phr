...
暗号ZIP/XML取込用パスワード管理画面

### 背景

- XML取込 `scripts/from_medical/02_import_xml.py` は、暗号ZIPを検出した場合に `work_other.medi_zip_passwords` からパスワードを解決している。
- 現行の解決キーは `ZIP_SHA256`、`ZIP_NAME`、`FACILITY` で、`FACILITY` では `file_receipts.facility_code`、`submitter_facility_code`、受領フォルダ名を候補として参照する。
- パスワードが見つからない場合は `ZIP_PASSWORD_NOT_FOUND`、合わない場合は `ZIP_DECRYPT_FAILED` として取込エラーに残る。
- そのため、新規テーブルを先に作るより、まず既存取込処理が参照している `work_other.medi_zip_passwords` を画面から管理できるようにするのが最短である。

### 方針

- 初期実装では、既存テーブル `work_other.medi_zip_passwords` を管理対象にする。
...
- `ZIP_PASSWORD_NOT_FOUND` / `ZIP_DECRYPT_FAILED` の取込エラーから、パスワード登録または編集へ進める導線を作る。
...
5. `ZIP_NAME` ではZIPファイル名、`ZIP_SHA256` ではZIP sha256を扱う。
6. 健診機関コードや受領フォルダ名は、可能であればalias/健診機関マスタ検索から選択できるようにする。
7. 取込エラー画面または処理実行結果から、ZIP名、sha256、施設コード、受領フォルダ名をプリセットして登録へ進める。
8. `ZIP_DECRYPT_FAILED` では既存登録が間違っている可能性が高いため、対象候補を表示して編集へ誘導する。
