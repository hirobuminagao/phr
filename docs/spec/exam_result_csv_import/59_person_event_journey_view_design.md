...
健診機関の指定は共通の健診機関選択モーダルを使用する。指定した健診機関IDが、予約サイトの施設対応後ID、受領ledger、active caseのいずれかに存在する対象者を表示する。予約と実受診の施設が異なる履歴を消さないため、初期版では3工程のいずれかに該当する条件とする。
...
件数カードはevent全体を集計し、ページ内件数から作らない。一覧は30件/ページのサーバ側ページングとし、健診結果工程はページ内のsubscriberだけを追加集計する。

初期実装では一覧に結果受領とcase・出力を追加する。受領はledger件数、source種別、最新更新日時、NG件数を表示する。caseはactive件数と最新caseへのリンク、受診日、健診機関、check/readiness/XML状態、出力リスト・出力済み件数を表示する。複数受診は件数を保ったまま表示し、最新1件だけに潰して集計しない。
