...
### Facility Folder Resolution for Corporate Bundled Receipts

同一法人などの都合で、受領時は1つのフォルダまたは1つのZIPに複数健診機関のXML/CSVがまとめられる場合がある。
この場合でも、scan/import/case checkまでは1つの受領単位から通常どおり処理してよい。
ただし、HIA提出用XMLの内容と出力先は、受領フォルダ名ではなく `exam_export_cases.exam_facility_id` を正とする。

`04_export_hia_xml.py` は、caseの `exam_facility_id` と健診機関マスタからXML内の健診機関コード・名称を生成する。
出力先フォルダは、まず `phr_master.medical_folder_aliases` の `event_id + exam_facility_id` に紐づく有効aliasから解決し、`dst_folder_norm` があればそれを使用し、なければ `src_folder_raw` を使用する。
aliasから解決できない場合だけ、従来どおり構成元sourceの `relative_path` 先頭フォルダ名を使用する。

この方針により、法人まとめZIPから取り込まれたcaseでも、最終出力は各健診機関フォルダ配下の `03_健診結果（アップロードデータ）` へ分ける。
XML内の健診機関コードと出力先フォルダは、caseの健診機関を基準に揃える。
構成元sourceのフォルダが1つにまとまっていても、それ自体をエラーにはしない。

今後の課題として、構成元sourceのフォルダとcaseの健診機関が一致しない場合を「許容して出力先をcase基準に補正する」か「不整合としてエラーにする」かを、イベントまたはalias設定で選択可能にする。
初期既定は、納品可能数を増やす目的を優先し、caseの健診機関から出力先を解決できる場合は許容する。
ただし、aliasが複数候補になり一意に決められない、またはcaseの健診機関が未確定の場合はエラーとして止める。
...
2026-08-31時点の管理画面では、出力先を `確認` / `本番` のトグルで選択する。`本番` を選択した場合は梱包方式を `健診機関・受診月別` に固定し、`リスト全員を単一DATA` は選択不可にする。`確認` の場合だけ2方式を切り替えられる。単一DATAの送付元健診機関番号の画面初期値は `1322100106` とし、空のままでは実行しない。

画面上の表示制御だけを安全境界にせず、サーバ側と `04_export_hia_xml.py` でも `official + single_data` を拒否または標準方式へ強制する。出力リスト名は詳細画面の変更ボタンからモーダルで更新し、直接編集可能な常設フォームにはしない。
...
mapping対象がない、またはmapping値がNULLの場合は、CSV取込時に健診年度末日の満年齢を求め、40～74歳を `10/010`、それ以外を `40/990` として不足値を補完する。
`event.age_reference_date` は予約時のコース分け等に使う運用日付であり、特定健診対象判定および報告区分/プログラムコード補完の年齢基準日としては使わない。
`event_id = 2` は2026年度定期健診であり、年齢基準日は年度末 `2027-03-31` を使用する。
