...
- テーブル自体は作成済み
- 本来想定している「受領CSVをテンプレート定義に基づいて staging 用カラムへ格納する」処理は未整備
...
受領原本そのものの保持は、入力CSV、archive、ETL実行記録で担保する前提とし、`staging_subscribers_fund` では norm 格納を基本とする。
raw は全項目を機械的に保持する前提ではなく、mapping上どうしても raw でしか受けられない項目、または運用上必要性が明確な項目に限って保持する。

一方で、出所追跡や行特定に必要な以下の入力起点情報は保持する。
...
- raw は全項目を常設保持しない
- norm格納を基本とする
- raw は mapping上どうしても raw でしか受けられない項目、または運用上必要性が明確な項目に限って保持する
- 旧DDL上の raw寄りカラムは、原則として norm側へ再配置し、raw列としての再作成は例外項目に限定する
- 主値は `*_norm` とする
- 照合用補助値は `*_match` とする
- 出所追跡は `src_*` 系で担保する
...
- raw は全項目を保持する前提ではなく、norm格納を基本とし、mapping上どうしても raw でしか受けられない項目または運用上必要性が明確な項目に限って保持する方針とする
