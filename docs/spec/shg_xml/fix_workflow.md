...
### final動機づけ支援のアウトカム合計ポイント0ブロック削除

基幹システム取込時のエラー回避のため、以下条件を満たす場合に限り、アウトカム合計ポイント0の既存XMLブロック削除fixを検討する。

条件:

- `report_code = 22`
- 保健指導区分が動機づけ支援
- `code = 1042001060` を持つ `entryRelationship[typeCode="COMP"]` が存在する
- アウトカム合計ポイント値が `0`

背景:

- 動機づけ支援ではアウトカム合計ポイントが0であること自体は自然である
- ただし、基幹システム取込時に0ポイントブロックが存在するとエラーになるケースが確認されている
- 以前は許容されていたが、基幹側チェックが厳格化された可能性がある

方針:

- 利用券fixとは別系統のfixとして扱う
- XML単位ループ内で実行する
- DB比較ではなくXML内条件で判定する
- 削除処理自体は `scripts/lib/xml/delete.py` へ外出しする
- SHG側では削除条件判定と削除対象特定のみを担当する
- 削除対象は `90060` section 内の `code = 1042001060` を持つ `entryRelationship[typeCode="COMP"]` とする
- `observation` 単体削除ではなく、対応する `entryRelationship` を丸ごと削除する
- section 全体削除や entry 全体削除は行わない
- 削除条件の判定はSHG側で行い、`delete.py` は削除操作のみを担当する

注意:

- これは既存XMLブロック削除であり、通常の属性値更新より強いfixである
- 推定で削除してはならない
- XPath文字列のみで削除対象を決めない
- 削除対象特定時は、実際の XML Element と parent Element を保持して削除する
- 新規XMLブロック作成は行わない
- 削除対象は、`90060` section 配下で `observation/code = 1042001060` を持つ `entryRelationship[typeCode="COMP"]` に固定する
