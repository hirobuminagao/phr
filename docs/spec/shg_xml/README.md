...
4. ZIP展開後の作業フォルダ削除を実装する
5. `結_腹囲体重` の表示ルール（`目標なし` / `-` を含む）を整理する
6. その後、XML自動修正（fix）ロジックへ拡張する

### fixロジック検討時の前提

- 利用券整理番号と利用券有効期限は、DB側を正としてXML修正対象にできる
- 一般カテゴリは「目標なしなのに達成/未達成」がルール違反なので、自動で `非目標` へ寄せる余地がある
- ただし、目標があるものに対して達成 / 未達成のどちらが正しいかは支援内容依存のため、スクリプトでは判断しない
- 腹囲体重は一般カテゴリと異なり、実測で上位結果が出たらその上位結果で報告する特殊ルールとして扱う
- 喫煙は一般カテゴリと同じ思想ではあるが、非喫煙者用の結果コードが別であるため、別分岐が必要

### 再開時の合言葉

- 「SHGチェックは最低限動作済み。次は初回面談方式、動機づけ目標、ZIP後処理、CSVのsource列を直す」