...
CSVマッピングラボは、全列を眺めるだけではなく、機械判定後に残った確認対象を潰す作業画面として育てる。
...
集計カードを押すと、その状態を現在の絞り込み条件へ追加する。CSVが1000列を超える施設では、全列表示よりも「REVIEWだけ見る」「未確認だけ見る」を主な導線にする。

## JLAC構成コードでの原値保持

CSVには、提出XMLで使う判定値とは別に、健診機関の検査機器や帳票から出た原値が入ることがある。

例として聴力では、健診機関CSVに `10`、`25`、`35` などのdB数値が入る。一方、特定健診XMLで使う既存の聴力項目は、所見有無や判定コードとして扱うため、dB原値の保持先としては意味がずれる。

この場合は、提出XMLの収載項目に限定せず、JLAC10の5要素から内部保持用の `namecode` を構成して `item_master` に追加する方針とする。

聴力dB原値の候補:

| 保持したい値 | JLAC10構成namecode | 備考 |
| --- | --- | --- |
| 聴力 右 1000Hz dB | `9D100163100000001` | `9D100` 標準聴力検査 + `1631` 右1000Hz + `000` + `000` + `01` 定量値 |
| 聴力 右 4000Hz dB | `9D100163200000001` | `1632` 右4000Hz |
| 聴力 左 1000Hz dB | `9D100163500000001` | `1635` 左1000Hz |
| 聴力 左 4000Hz dB | `9D100163600000001` | `1636` 左4000Hz |

`item_master` に追加する場合の扱い:

- `xml_value_type`: `PQ`。
- `display_unit` / `ucum_unit`: `dB` 系。
- `annex2_legal_report_flag`: `0`。画面説明では「提出XML出力対象外」と表現する。
- `notes`: CSV原値保持用。提出XMLには出力しない。

この追加項目は `exam_item_values` に保持し、case詳細やCSV補完確認では参照できるようにする。提出XMLへ出す聴力は、従来の提出用CD項目を使う。
