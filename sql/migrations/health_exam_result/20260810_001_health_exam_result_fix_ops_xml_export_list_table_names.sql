-- Repair migration for environments that applied the former
-- 20260806_002 migration before the export-list tables were renamed with
-- the ops_ prefix.
--
-- Do not edit the already-published 20260806_002 migration for deployed
-- environments. This migration reconciles either state:
--   - old tables exist: xml_export_lists / xml_export_list_cases
--   - new tables do not exist yet
--   - xml_export_zips may or may not already have xml_export_list_id

SET @has_old_lists := (
  SELECT COUNT(*)
  FROM information_schema.tables
  WHERE table_schema = 'health_exam_result'
    AND table_name = 'xml_export_lists'
);
SET @has_new_lists := (
  SELECT COUNT(*)
  FROM information_schema.tables
  WHERE table_schema = 'health_exam_result'
    AND table_name = 'ops_xml_export_lists'
);

SET @sql := IF(
  @has_old_lists = 1 AND @has_new_lists = 0,
  'RENAME TABLE `health_exam_result`.`xml_export_lists` TO `health_exam_result`.`ops_xml_export_lists`',
  'DO 0'
);
...
SET @has_old_cases := (
  ...
  FROM information_schema.tables
  WHERE table_schema = 'health_exam_result'
    AND table_name = 'xml_export_list_cases'
);
SET @has_new_cases := (
  SELECT COUNT(*)
  FROM information_schema.tables
  WHERE table_schema = 'health_exam_result'
    AND table_name = 'ops_xml_export_list_cases'
);

SET @sql := IF(
  @has_old_cases = 1 AND @has_new_cases = 0,
  'RENAME TABLE `health_exam_result`.`xml_export_list_cases` TO `health_exam_result`.`ops_xml_export_list_cases`',
  'DO 0'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
CREATE TABLE IF NOT EXISTS `health_exam_result`.`ops_xml_export_lists` (
...
CREATE TABLE IF NOT EXISTS `health_exam_result`.`ops_xml_export_list_cases` (
...
SET @has_zip_list_column := (
  SELECT COUNT(*)
  FROM information_schema.columns
  WHERE table_schema = 'health_exam_result'
    AND table_name = 'xml_export_zips'
    AND column_name = 'xml_export_list_id'
);

SET @sql := IF(
  @has_zip_list_column = 0,
  'ALTER TABLE `health_exam_result`.`xml_export_zips` ADD COLUMN `xml_export_list_id` bigint unsigned DEFAULT NULL COMMENT ''このZIPを作成した出力リスト。CLI直接出力時はNULL'' AFTER `etl_run_id`',
  'DO 0'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @has_zip_list_index := (
  SELECT COUNT(*)
  FROM information_schema.statistics
  WHERE table_schema = 'health_exam_result'
    AND table_name = 'xml_export_zips'
    AND index_name = 'idx_xml_export_zips_export_list'
);

SET @sql := IF(
  @has_zip_list_index = 0,
  'ALTER TABLE `health_exam_result`.`xml_export_zips` ADD KEY `idx_xml_export_zips_export_list` (`xml_export_list_id`)',
  'DO 0'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @has_zip_list_fk := (
  ...
  WHERE constraint_schema = 'health_exam_result'
    AND table_name = 'xml_export_zips'
    AND constraint_name = 'fk_xml_export_zips_export_list'
);

SET @sql := IF(
  @has_zip_list_fk = 0,
  'ALTER TABLE `health_exam_result`.`xml_export_zips` ADD CONSTRAINT `fk_xml_export_zips_export_list` FOREIGN KEY (`xml_export_list_id`) REFERENCES `health_exam_result`.`ops_xml_export_lists` (`xml_export_list_id`)',
  'DO 0'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
